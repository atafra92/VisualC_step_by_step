...
namespace QueryBinaryTree
{
    class Program
    {
        static void doWork()
        {
            ...
            var allEmployees = from e in empTree/*.ToList()  - cache the data and retreive it's copy when iterating over a collection*/ 
                               select e;
            ...
            //add a new employee to a IEnumerable<T> allEmployees
            empTree.Insert(new Employee { Id = 7, FirstName = "Ante", LastName = "Martic", Department = "IT" });
            Console.WriteLine();
